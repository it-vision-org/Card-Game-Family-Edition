...
class _SessionLobbyPageState extends ConsumerState<SessionLobbyPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: sessionAsync.when(
        ...
        data: (session) {
          final joinedPublicIds = session.participants.map((p) => p.publicId).toSet();
          final missingMembers = groupAsync.value?.members
                  .where((m) => !joinedPublicIds.contains(m.publicId))
                  .toList() ??
              const [];
          final requiredPowerCards = gameSettingsAsync.value?.powerCardsPerPlayer ?? 0;

          final playersStillPicking = session.participants
              .where((p) => p.powerCards.length != requiredPowerCards)
              .map((p) => p.displayName)
              .toList();
          final canStart = session.participants.length >= 2 && playersStillPicking.isEmpty;
          ...
          return Padding(
            ...
            child: Column(
              ...
              children: [
                Text('كود الجلسة', style: Theme.of(context).textTheme.titleMedium, textAlign: TextAlign.center),
                ...
                Text('اللاعبين', style: Theme.of(context).textTheme.titleMedium),
                const SizedBox(height: 8),
                ...session.participants.map((p) {
                  ...
                  return ListTile(
                    ...
                    subtitle: requiredPowerCards > 0
                        ? Text('بطاقات القوة: ${p.powerCards.length} / $requiredPowerCards')
                        : null,
                    trailing: requiredPowerCards > 0
                        ? (isSubmitting
                            ...
                            : TextButton(
                                onPressed: () => _choosePowerCards(p.id, p.displayName, requiredPowerCards),
                ...
                if (missingMembers.isNotEmpty) ...[
                  ...
                  Text('ضيف لاعب (نفس الهاتف)', style: Theme.of(context).textTheme.titleSmall),
                  Wrap(
                    spacing: 8,
  ...
  Future<void> _choosePowerCards(String participantId, String participantName, int requiredCount) async {
    ...
    try {
      await ref.read(sessionControllerProvider(widget.sessionId).notifier).choosePowerCards(participantId, ids);
    } on ApiException catch (e) {
      if (mounted) {
  ...
  Future<void> _start() async {
    ...
    try {
      await ref.read(sessionControllerProvider(widget.sessionId).notifier).start();
    } on ApiException catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.message)));
      }
